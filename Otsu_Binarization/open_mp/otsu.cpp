...
void scan_parallel(double* &cum_histogram, double* &cum_sum){
    ...
    for(d = 1; d<MAX_INTENSITY; d<<=1){
        ...
        #pragma omp parallel for num_threads(NT) shared(cum_histogram, cum_sum)
...
int otsu_binarization(uint8_t* &gray_img, int width, int height){
    ...
    #pragma omp parallel for num_threads(NT) shared(histogram) private(i)
    for(i=0; i< MAX_INTENSITY; i++){
        histogram[i] = 0.0;
    }

    //TEST ??
    //#pragma omp parallel for num_threads(NT) shared(histogram, gray_img)
    for (i=0; i<width* height; i++) {
        //#pragma omp atomic
        histogram[gray_img[i]] += 1.0;
    }

    #pragma omp parallel for num_threads(NT) shared(histogram) reduction(+: total_sum)
    for(i=0; i<MAX_INTENSITY; i++){
    ...
    #pragma omp parallel for num_threads(NT) private(p1_num, p2_num, p1_sum, p2_sum, p1_mu, p2_mu, var, mu_diff) shared(cum_histogram, threshold, total_pts, total_sum)
    ...
    #pragma omp parallel for num_threads(NT) shared(max_vars) reduction(max: max_var)
    for(thresh=0; thresh<MAX_INTENSITY; thresh++){
	max_var = std::max(max_var, max_vars[thresh]);
	}
    #pragma omp parallel for num_threads(NT) shared(max_vars)
    for(thresh=0; thresh<MAX_INTENSITY; thresh++){
	if(max_vars[thresh]==max_var) threshold=thresh;
}

    #pragma omp parallel for num_threads(NT) shared(new_img, threshold)
...
int main(int argc, char **argv){
    printf("Starting off ... \n");
    const char *img_file = "is_test1.jpg";
